...
class New:
    ...
    def task_guide(self):
        ...
        for _ in range(3):
            ...
            self.game.timer(90, "等待BOSS充能")
            print("点击boss")
            self.game.switch_auto("auto_green")
            self.action.click(293,331)  # 点击boss出现的地点
            time.sleep(1)
        ...
        for _ in range(3):
            if self.image_tool.text("流浪"):
                self.image_tool.text("莉迪亚", click_times=3)
                self.action.click(542, 326)  # 左箭头
                self.action.click(339, 273)  # 流浪者工会
                self.action.click(383, 577)  # 确认
                self.game.wait_loaded("莉迪亚", load="text")
                self.image_tool.text("莉迪亚", click_times=3)
                self.action.click(245, 988)  # 城镇图标
                self.action.click(383, 572)  # 去镇上
                self.game.wait_loaded("ruby")
